...
def xAxis():
    ...
    for i in range(50):
        ...
        px_sim_axis.append((noise_count-damage_count)/(noise_count))
...
def tests(indcs = [0,1,2,3,4,5,6,7], k = 0):
    ...
    for method in all_methods:
        ...
        ax = plt.axes()
        plt.plot(x_axis, ave, method[2], linestyle = '--' if '_random' in method[1] else '-', label = method[5])
        plt.fill_between(x_axis, ave-std_error, ave+std_error, alpha=0.05 if '_random' in method[1] else .5, edgecolor=method[2], facecolor=method[2])
        plt.legend(loc = 0)
        plt.yticks(np.arange(80, 102.5, 2.5))
        plt.xticks(np.arange(0, 65, 5))
        plt.title(title);plt.xlabel('Percent Noise (%)'.format(int(total_segs)));plt.ylabel('AUC (%)'); plt.xlim([0, 40]), plt.ylim(80, 100)
    fig.savefig('../Paper Figures/{}'.format(title.replace(" ", "")))
...
class noise():
    def __init__(self, run_num = 0, model_type = 'px', random = False, dilate = True, minimal = False):
        ...
        self.p = 'ObjectNoiseProgress{}/'.format(self.te)
